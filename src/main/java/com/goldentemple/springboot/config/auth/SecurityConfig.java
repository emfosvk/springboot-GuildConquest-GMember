    ...
    protected void configure(HttpSecurity httpSecurity) throws Exception{
        httpSecurity.csrf().disable()
                ...
                .and().authorizeRequests()
                    .antMatchers("/vw/**.vw", "/", "/css/**", "/img/**", "/js/**", "/h2-console/**", "/profile", "/book/api/**.api").permitAll()
                    .antMatchers("/comn/api/**").hasAnyRole(Role.MANAGER.name(), Role.MASTER.name(), Role.ADMIN.name())
